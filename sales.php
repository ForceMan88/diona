...
class sales extends CI_Model
{
    ...
    public function getSelectSales($data, $real_estate, $limit = '', $offset = '')
    {
        ...
        if (isset($data['real_estate']) and $data['real_estate'] != 'flat' and $data['real_estate'] != 'room') {
        ...
        if (isset($data['count_room']) and !empty($data['count_room'])) {
        ...
        if (isset($data['sorted']) and !empty($data['sorted'])) {
            $query .= " order by " . mysql_real_escape_string($data['sorted']).' DESC';
        }
...
////        $allCountResult = $this->db->query($query);
...
//        $res['count_item'] =  $allCountResult->row()->count ;
//        var_dump($res);
